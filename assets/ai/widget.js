...
import { getNukraxResponse } from './responder.js';
...
if (!document.getElementById(ROOT_ID)) {
  ...
  #${ROOT_ID} {
    position:fixed; z-index:9500; right:26px; bottom:48px;
    font-family:var(--font-b, sans-serif); color:var(--text,#D3DBDD);
  }
  @media(max-width:600px){ #${ROOT_ID}{ right:16px; bottom:calc(64px + env(safe-area-inset-bottom, 0px)); } }

  /* ── Back to top ── */
  .nkx-top-btn{
    position:absolute; right:2px; bottom:82px;
  ...
  .nkx-fab-label{
    font-family:var(--font-b,sans-serif); font-size:13px; font-weight:500;
    color:var(--white,#F2F5F5); background:var(--panel,#0A0F13); border:1px solid var(--line-lit,#2A353C);
    padding:10px 16px; border-radius:20px; white-space:nowrap; box-shadow:0 6px 20px rgba(0,0,0,.4);
  ...
  /* ── Popup panel (Claude-style rounded card) ── */
  .nkx-panel{
    position:absolute; right:0; bottom:80px;
    width:380px; max-width:calc(100vw - 32px); height:min(580px, 74vh); height:min(580px, 74dvh);
    background:var(--panel,#0A0F13); border:1px solid var(--line,#1B2328);
  ...
  @media(max-width:600px){
    .nkx-panel{ width:calc(100vw - 32px); height:min(76vh, 620px); height:min(76dvh, 620px); border-radius:20px; }
  }

  .nkx-head{
    display:flex; align-items:center; justify-content:space-between;
    padding:15px 16px; border-bottom:1px solid var(--line,#1B2328);
  ...
  .nkx-head-title{ font-size:14px; font-weight:600; color:var(--white,#F2F5F5); letter-spacing:.01em; }
  ...
  .nkx-head-btn{
    font-family:var(--font-b,sans-serif); font-size:12px; font-weight:500;
    color:var(--accent,#8FB8C4); background:var(--panel,#0A0F13); border:1px solid var(--line-lit,#2A353C);
    padding:7px 13px; border-radius:16px; cursor:pointer; display:flex; align-items:center; gap:5px;
    transition:border-color .25s, color .25s, background .25s;
  }
  .nkx-head-btn:hover{ border-color:var(--accent,#8FB8C4); color:var(--white,#F2F5F5); }
  .nkx-icon-btn{
    width:28px; height:28px; border-radius:50%; background:transparent; border:1px solid transparent;
    color:var(--muted,#8A9BA3); cursor:pointer; display:flex; align-items:center; justify-content:center;
    transition:border-color .2s, color .2s, background .2s;
  }
  .nkx-icon-btn:hover{ border-color:var(--line-lit,#2A353C); color:var(--white,#F2F5F5); background:rgba(255,255,255,.04); }

  /* ── Body / messages ── */
  .nkx-body{ position:relative; flex:1; overflow-y:auto; padding:18px 16px; display:flex; flex-direction:column; gap:14px; }
  .nkx-body::-webkit-scrollbar{ width:3px; }
  .nkx-body::-webkit-scrollbar-thumb{ background:var(--line-lit,#2A353C); }

  .nkx-msg{ max-width:86%; font-size:14px; line-height:1.55; animation:nkx-msg-in .32s var(--ease-out,ease) both; }
  @keyframes nkx-msg-in{ from{ opacity:0; transform:translateY(8px) scale(.98); } to{ opacity:1; transform:translateY(0) scale(1); } }
  .nkx-msg-user{ align-self:flex-end; background:var(--accent,#8FB8C4); padding:10px 15px; border-radius:18px 18px 4px 18px; color:var(--black,#030507); font-weight:500; }
  .nkx-msg-ai{ align-self:flex-start; }
  .nkx-msg-ai-label{ font-family:var(--font-m,monospace); font-size:9.5px; letter-spacing:.12em; text-transform:uppercase; color:var(--accent,#8FB8C4); margin-bottom:5px; display:block; }
  .nkx-msg-ai-text{ color:var(--text,#D3DBDD); }

  .nkx-typing{ display:flex; align-items:center; gap:5px; align-self:flex-start; padding:10px 14px; background:rgba(143,184,196,0.06); border:1px solid rgba(143,184,196,0.14); border-radius:16px 16px 16px 4px; animation:nkx-msg-in .28s var(--ease-out,ease) both; }
  .nkx-typing span{ width:6px; height:6px; border-radius:50%; background:var(--accent,#8FB8C4); animation:nkx-bounce 1s ease-in-out infinite; }
  .nkx-typing span:nth-child(2){ animation-delay:.16s; }
  .nkx-typing span:nth-child(3){ animation-delay:.32s; }
  @keyframes nkx-bounce{ 0%,60%,100%{ transform:translateY(0) scale(1); opacity:.5; } 30%{ transform:translateY(-5px) scale(1.15); opacity:1; } }

  .nkx-empty{ display:flex; flex-direction:column; gap:16px; align-items:flex-start; padding-top:14px; }
  .nkx-empty-title{ font-size:16px; color:var(--white,#F2F5F5); font-weight:600; }
  .nkx-empty-sub{ font-size:12.5px; color:var(--muted,#8A9BA3); }
  .nkx-chip-row{ display:flex; flex-wrap:wrap; gap:8px; }
  .nkx-chip{
    font-size:12.5px; color:var(--text,#D3DBDD); background:var(--black,#030507);
    border:1px solid var(--line,#1B2328); padding:8px 14px; border-radius:20px; cursor:pointer;
    transition:border-color .2s, color .2s;
  }
  .nkx-chip:hover{ border-color:var(--accent,#8FB8C4); color:var(--white,#F2F5F5); }

  /* ── Input (Claude-style rounded pill) ── */
  .nkx-input-wrap{ border-top:1px solid var(--line,#1B2328); padding:12px 14px 14px; background:var(--black,#030507); position:relative; }
  .nkx-menu-row{ display:flex; align-items:center; justify-content:space-between; margin-bottom:9px; }
  .nkx-dots-btn{ width:26px; height:26px; border-radius:50%; background:transparent; border:1px solid var(--line,#1B2328); color:var(--muted,#8A9BA3); cursor:pointer; font-size:13px; line-height:1; display:flex; align-items:center; justify-content:center; transition:border-color .2s,color .2s; }
  ...
  .nkx-mode-menu{
    position:absolute; bottom:100%; left:14px; margin-bottom:8px;
    background:var(--panel,#0A0F13); border:1px solid var(--line-lit,#2A353C); border-radius:16px;
    min-width:200px; overflow:hidden; opacity:0; transform:translateY(6px); pointer-events:none;
    transition:opacity .2s, transform .2s; z-index:10;
  }
  .nkx-mode-menu.open{ opacity:1; transform:translateY(0); pointer-events:auto; }
  .nkx-mode-item{ padding:11px 14px; font-size:12.5px; cursor:pointer; display:flex; flex-direction:column; gap:2px; border-bottom:1px solid var(--line,#1B2328); }
  ...
  .nkx-input-row{ display:flex; align-items:center; gap:8px; background:var(--panel,#0A0F13); border:1px solid var(--line,#1B2328); border-radius:26px; padding:6px 6px 6px 16px; transition:border-color .2s; }
  .nkx-input-row:focus-within{ border-color:var(--accent,#8FB8C4); }
  .nkx-textarea{
    flex:1; resize:none; background:transparent; border:none;
    color:var(--white,#F2F5F5); font-family:var(--font-b,sans-serif); font-size:13.5px;
    padding:8px 0; max-height:90px; outline:none; line-height:1.4;
  }
  .nkx-textarea::placeholder{ color:var(--muted,#8A9BA3); }
  .nkx-send-btn{
    width:36px; height:36px; flex-shrink:0; border-radius:50%; background:var(--accent,#8FB8C4);
    border:none; color:var(--black,#030507); cursor:pointer; position:relative;
  ...
  .nkx-send-btn:active{ transform:scale(.88); }
  .nkx-send-btn:disabled{ opacity:.55; cursor:default; }
  .nkx-send-btn svg{ transition:opacity .15s, transform .15s; }
  .nkx-send-btn.loading svg{ opacity:0; transform:scale(.6); }
  .nkx-send-btn .nkx-spinner{
    position:absolute; inset:0; border-radius:50%; opacity:0; pointer-events:none;
    border:2px solid rgba(3,5,7,0.25); border-top-color:var(--black,#030507);
  }
  .nkx-send-btn.loading .nkx-spinner{ opacity:1; animation:nkx-spin .7s linear infinite; }
  @keyframes nkx-spin{ to{ transform:rotate(360deg); } }

  /* ── Tutorial overlay ── */
  .nkx-tutorial{
    position:absolute; inset:0; background:var(--panel,#0A0F13); z-index:20;
    display:flex; flex-direction:column; justify-content:space-between; padding:24px 22px;
  ...
  .nkx-tut-icon{
    width:46px; height:46px; border-radius:16px; border:1px solid var(--line-lit,#2A353C);
    display:flex; align-items:center; justify-content:center; font-size:19px; color:var(--accent,#8FB8C4);
    background:var(--black,#030507);
  }
  .nkx-tut-title{ font-size:17px; font-weight:600; color:var(--white,#F2F5F5); }
  ...
  .nkx-tut-footer{ display:flex; flex-direction:column; gap:16px; }
  ...
  .nkx-tut-continue{
    ...
    border:none; padding:11px 20px; border-radius:20px; cursor:pointer;
    ...
    <div class="nkx-fab-wrap">
      ...
      <button class="nkx-fab" data-fab type="button" aria-label="Open Lanux chat">
        ...
        <span class="nkx-fab-inner">
          <img src="${location.pathname.includes('/ea/') ? '../assets/ai.png' : 'assets/ai.png'}" alt="" onerror="this.style.display='none'"/>
        </span>
      </button>
    </div>
  `;
  ...
  function appendUserBubble(text) {
    const empty = body.querySelector('.nkx-empty');
  ...
  function appendAssistantBubble() {
    ...
    el.innerHTML = `<span class="nkx-msg-ai-label">LANUX</span><span class="nkx-msg-ai-text"></span>`;
    body.appendChild(el);
    return el.querySelector('.nkx-msg-ai-text');
  }

  // Letter-by-letter reveal for assistant replies, duration scales with
  // length but is capped so long answers don't take forever to appear.
  function typeWriter(el, text) {
    return new Promise(resolve => {
      const duration = Math.min(1800, Math.max(280, text.length * 13));
      const start = performance.now();
      function frame(now) {
        const progress = Math.min(1, (now - start) / duration);
        const chars = Math.max(1, Math.floor(progress * text.length));
  ...
  async function sendMessage() {
    ...
    if (mode === 'nukrax') {
      // Fully local — no network call, no daily limit, works on any host.
      const delay = 280 + Math.random() * 220;
      await new Promise(r => setTimeout(r, delay));
      reply = getNukraxResponse(text);
    } else {
      // General AI: try the live model; gracefully fall back to the local
      // engine if the backend isn't reachable, so something useful still
      // comes back instead of a dead-end error.
      try {
        ...
        reply = data.reply || getNukraxResponse(text);
      } catch (err) {
        reply = getNukraxResponse(text);
      }
    }
    ...
    const textEl = appendAssistantBubble();
    sendBtn.disabled = false;
    sendBtn.classList.remove('loading');
    await typeWriter(textEl, reply);
  }
}
