...
// Weighted overlap: returns both a weighted score and the count of
// distinct real keywords that matched, so callers can judge relevance
// (one shared filler-ish word ≠ a real match; several specific words do).
function overlapDetail(queryWords, targetText) {
  const targetWords = new Set(normalize(targetText));
  const seen = new Set();
  let score = 0, matched = 0;
  queryWords.forEach(w => {
    if (targetWords.has(w) && !seen.has(w)) {
      seen.add(w);
      matched++;
      score += w.length >= 7 ? 3 : w.length >= 5 ? 2 : 1;
    }
  });
  return { score, matched };
}

// Confidence bar scales with message length — a short, precise question
// needs less evidence than a long one, where a single stray shared word
// is far more likely to be a coincidence rather than a real match.
function isConfident(score, matched, queryLen) {
  if (queryLen <= 4) return matched >= 1 && score >= 2;
  if (queryLen <= 8) return matched >= 2 && score >= 3;
  return matched >= 3 && score >= 4;
}
...
export function getNukraxResponse(rawMessage) {
  ...
  // ── EA-specific match (an explicit name mention is decisive on its own) ──
  let bestEa = null, bestEaScore = 0, bestEaMatched = 0, eaNameMentioned = false;
  NUKRAX_KNOWLEDGE.eas.forEach(ea => {
    const { score, matched } = overlapDetail(queryWords, ea.name + ' ' + ea.desc + ' ' + ea.tag);
    const nameMentioned = lower.includes(ea.name.toLowerCase().split(' ')[0]);
    const adjustedScore = score + (nameMentioned ? 6 : 0);
    if (adjustedScore > bestEaScore) {
      bestEaScore = adjustedScore; bestEaMatched = matched; bestEa = ea; eaNameMentioned = nameMentioned;
    }
  });

  // ── FAQ match ──
  let bestFaq = null, bestFaqScore = 0, bestFaqMatched = 0;
  NUKRAX_KNOWLEDGE.faq.forEach(item => {
    const { score, matched } = overlapDetail(queryWords, item.q + ' ' + item.a);
    if (score > bestFaqScore) { bestFaqScore = score; bestFaqMatched = matched; bestFaq = item; }
  });

  const eaConfident = bestEa && (eaNameMentioned || isConfident(bestEaScore, bestEaMatched, queryWords.length));
  const faqConfident = bestFaq && isConfident(bestFaqScore, bestFaqMatched, queryWords.length);
