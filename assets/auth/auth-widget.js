...
function injectStyles() {
  ...
  s.textContent = `
    #nkx-auth-widget{position:fixed;top:16px;right:16px;z-index:600;display:flex;align-items:center;gap:8px;
      font-family:var(--font-b,sans-serif)}
    @media(max-width:560px){#nkx-auth-widget{top:10px;right:10px}}
    ...
    @media (hover:hover) and (pointer:fine){.nkx-w-btn:hover{border-color:var(--accent,#8FB8C4);color:var(--white,#F2F5F5)}}
    ...
    @media (hover:hover) and (pointer:fine){.nkx-w-login:hover{box-shadow:0 0 14px 3px var(--accent,#8FB8C4),0 0 4px var(--accent,#8FB8C4)}}
    ...
    @media (hover:hover) and (pointer:fine){.nkx-w-dropdown a:hover,.nkx-w-dropdown button:hover{background:var(--line,#1B2328);color:var(--white,#F2F5F5)}}
    ...
    @media (hover:hover) and (pointer:fine){.nkx-eye:hover{color:var(--accent,#8FB8C4)}}
    ...
    @media (hover:hover) and (pointer:fine){.nkx-auth-submit:hover{opacity:.9}}
    ...
    @media (hover:hover) and (pointer:fine){.nkx-forgot-link:hover{color:var(--accent,#8FB8C4)}}
    ...
    @media (hover:hover) and (pointer:fine){.nkx-auth-oauth button:hover{border-color:var(--accent,#8FB8C4);color:var(--white,#F2F5F5)}}
    .nkx-auth-close{position:absolute;top:14px;right:16px;background:none;border:none;color:var(--muted,#8A9BA3);
      font-size:20px;cursor:pointer;line-height:1}
    @media (hover:hover) and (pointer:fine){.nkx-auth-close:hover{color:var(--white,#F2F5F5)}}
    ...
    @media (hover:hover) and (pointer:fine){.nkx-auth-back:hover{color:var(--accent,#8FB8C4)}}
    ...
    .nkx-sent{padding:8px 4px 4px;text-align:center}
    .nkx-sent-icon{width:64px;height:64px;margin:4px auto 18px;position:relative}
    .nkx-sent-icon svg{width:100%;height:100%;overflow:visible}
    .nkx-sent-ring{fill:none;stroke:var(--accent,#8FB8C4);stroke-opacity:.18;stroke-width:2}
    .nkx-sent-circle{fill:none;stroke:var(--accent,#8FB8C4);stroke-width:2;stroke-linecap:round;
      stroke-dasharray:151;stroke-dashoffset:151;
      animation:nkx-draw-circle .5s cubic-bezier(.65,0,.35,1) .05s forwards}
    .nkx-sent-check{fill:none;stroke:var(--accent,#8FB8C4);stroke-width:3;stroke-linecap:round;stroke-linejoin:round;
      stroke-dasharray:32;stroke-dashoffset:32;
      animation:nkx-draw-check .35s cubic-bezier(.65,0,.35,1) .45s forwards}
    .nkx-sent-title{font-family:var(--font-m,monospace);font-size:13.5px;letter-spacing:.06em;text-transform:uppercase;
      color:var(--white,#F2F5F5);margin-bottom:10px;opacity:0;transform:translateY(6px);
      animation:nkx-sent-rise .5s cubic-bezier(.22,1,.36,1) .55s forwards}
    .nkx-sent-sub{font-size:13px;line-height:1.55;color:var(--muted,#8A9BA3);max-width:280px;margin:0 auto;
      opacity:0;transform:translateY(6px);
      animation:nkx-sent-rise .5s cubic-bezier(.22,1,.36,1) .65s forwards}
    @keyframes nkx-draw-circle{to{stroke-dashoffset:0}}
    @keyframes nkx-draw-check{to{stroke-dashoffset:0}}
    @keyframes nkx-sent-rise{to{opacity:1;transform:translateY(0)}}
    @media (prefers-reduced-motion: reduce){
      .nkx-sent-circle,.nkx-sent-check,.nkx-sent-title,.nkx-sent-sub{animation:none;stroke-dashoffset:0;opacity:1;transform:none}
    }
...
function showEmailSentSuccess(modal, text) {
  modal.innerHTML = `
    <button class="nkx-auth-close" id="nkx-c">×</button>
    <div class="nkx-sent">
      <div class="nkx-sent-icon">
        <svg viewBox="0 0 52 52">
          <circle class="nkx-sent-ring" cx="26" cy="26" r="24"/>
          <circle class="nkx-sent-circle" cx="26" cy="26" r="24"/>
          <path class="nkx-sent-check" d="M15 27l7.2 7L38 18"/>
        </svg>
      </div>
      <h3 class="nkx-sent-title">Confirmation email sent</h3>
      <p class="nkx-sent-sub">${text}</p>
    </div>
  `;
...
function renderView(view, opts = {}) {
    ...
    modal.querySelector('#nkx-form').addEventListener('submit', async (e) => {
      ...
      if (view === 'signup') {
        ...
        const { error } = await supabase.auth.signUp({
          email, password,
          options: { emailRedirectTo: window.location.origin + BASE + 'setup.html' }
        });
        submitBtn.disabled = false;
        if (error) return setMsg(error.message, true);
        showEmailSentSuccess(modal, 'Click the link in your inbox to finish signing up.');
        return;
      }

      // login: verify password, then require a fresh email link as a second check
      ...
      await supabase.auth.signOut(); // don't finalize session until the email link is clicked
      const { error: otpError } = await supabase.auth.signInWithOtp({
        email,
        options: { shouldCreateUser: false, emailRedirectTo: window.location.origin + HOME_URL }
      });
      submitBtn.disabled = false;
      if (otpError) return setMsg(otpError.message, true);
      showEmailSentSuccess(modal, 'Click the link in your inbox to finish logging in.');
    });
    return;
  }
  ...
  if (view === 'forgot') {
    ...
    modal.querySelector('#nkx-forgot-form').addEventListener('submit', async (e) => {
      ...
      if (error) return setMsg(error.message, true);
      showEmailSentSuccess(modal, 'Click the link in your inbox to set a new password.');
    });
    return;
  }
...
function buildLoginWrap(user) {
  ...
  const isCommunityPage = window.location.pathname.replace(/^.*\//, '') === 'community.html';

  if (user) {
    const meta = user.user_metadata || {};
    const displayName = meta.username || meta.full_name || meta.user_name || meta.name || user.email;
    const avatarUrl = meta.avatar_url || meta.picture || '';
    const avatarInner = avatarUrl ? `<img src="${avatarUrl}" alt="">` : initials(displayName);

    wrap.innerHTML = isCommunityPage
  ...
  if (user) {
    if (isCommunityPage) {
      wrap.querySelector('#nkx-w-avatar-btn').addEventListener('click', () => {
