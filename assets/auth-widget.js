...
//
// It injects its own floating menu + profile widget (top-right corner)
// on every page — no markup changes needed on the page itself.
//
...
function injectStyles() {
  ...
  s.textContent = `
    ...
    .nkx-w-login{font-family:var(--font-m,monospace);font-size:11px;letter-spacing:.08em;text-transform:uppercase;
      background:rgba(10,15,19,.9);border:1px solid var(--line,#1B2328);backdrop-filter:blur(6px);color:var(--text,#D3DBDD);
      padding:9px 16px;border-radius:20px;cursor:pointer;white-space:nowrap;transition:.2s ease}
    .nkx-w-login:hover{border-color:var(--accent,#8FB8C4);color:var(--white,#F2F5F5)}
...
function renderWidget(user) {
  let root = document.getElementById('nkx-auth-widget');
  if (!root) {
  ...
  const profileBlockHtml = user ? (() => {
    ...
    `;
  })() : `<button class="nkx-w-login" id="nkx-w-login-btn">Log In</button>`;

  root.innerHTML = `
    <div style="position:relative">
      <div class="nkx-w-btn" id="nkx-w-menu-btn">${MENU_ICON}</div>
      <div class="nkx-w-dropdown" id="nkx-w-menu-dd">${menuLinksHtml}</div>
    </div>
    <div style="position:relative">${profileBlockHtml}</div>
  `;

  const menuBtn = root.querySelector('#nkx-w-menu-btn');
  const menuDd = root.querySelector('#nkx-w-menu-dd');
  menuBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    const isOpen = menuDd.classList.contains('open');
    closeAllDropdowns();
    if (!isOpen) menuDd.classList.add('open');
  });

  if (user) {
    const avatarBtn = root.querySelector('#nkx-w-avatar-btn');
    const avatarDd = root.querySelector('#nkx-w-avatar-dd');
    avatarBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      const isOpen = avatarDd.classList.contains('open');
      closeAllDropdowns();
      if (!isOpen) avatarDd.classList.add('open');
    });
    root.querySelector('#nkx-w-logout').addEventListener('click', async () => {
      await supabase.auth.signOut();
      window.location.href = HOME_URL;
    });
  } else {
    root.querySelector('#nkx-w-login-btn').addEventListener('click', () => openModal('login'));
  }
}
