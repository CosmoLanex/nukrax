...
export async function renderFeed(container, currentUser, filterUsername = null) {
  ...
  if (!posts || !posts.length) {
    container.innerHTML = `
      <div class="cm-empty-state">
        ...
        <button class="cm-empty-cta" id="cm-empty-post-btn">New Post</button>
      </div>`;
    container.querySelector('#cm-empty-post-btn')?.addEventListener('click', () => {
      document.getElementById('cm-fab-post')?.click();
    });
    return;
  }
      ...
      <article class="cm-post" data-post-id="${post.id}" data-author-id="${post.user_id}">
        ...
        <div class="cm-post-body">
          <div class="cm-post-head">
            ...
            ${isOwn ? `<button class="cm-post-edit" data-edit-post="${post.id}" title="Edit">Edit</button>` : ''}
  ...
  // Wire up edit
  container.querySelectorAll('.cm-post-edit').forEach(btn => {
    btn.addEventListener('click', () => {
      const postId = btn.dataset.editPost;
      const post = posts.find(p => p.id == postId);
      if (!post) return;
      openComposer(currentUser, () => renderFeed(container, currentUser, filterUsername), { editPost: post });
    });
  });
